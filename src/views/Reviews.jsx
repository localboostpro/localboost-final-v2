...
import { MessageSquare, Star, AlertCircle, Loader } from 'lucide-react';

export default function Reviews() {
  const { profile, reviews, loading } = useData();
  ...
  const ratingDistribution = [5, 4, 3, 2, 1].map(rating => ({
    ...
    percentage: reviewList.length > 0 ? (reviewList.filter(r => r.rating === rating).length / reviewList.length * 100) : 0
  }));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-[2.5rem] p-6 shadow-lg">
          ...
          <p className="text-sm text-slate-500 mt-1">{reviewList.length} avis</p>
        </div>

        <div className="bg-white rounded-[2.5rem] p-6 shadow-lg">
          <p className="text-sm font-bold text-slate-600 mb-2">Avis Positifs</p>
          <p className="text-4xl font-black text-green-600">
            {reviewList.filter(r => r.rating >= 4).length}
          </p>
        ...
        <div className="bg-white rounded-[2.5rem] p-6 shadow-lg">
          ...
          <p className="text-4xl font-black text-orange-600">
            {reviewList.filter(r => r.rating <= 3).length}
          </p>
