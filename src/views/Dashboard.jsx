...
export default function Dashboard({ stats, posts, onGenerate, profile }) {
  ...
  const fetchWeather = async (lat, lon) => {
      ...
      } catch (e) { console.error("Météo HS", e); }
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-4">
         <div>
            ...
            <p className="text-slate-500 font-medium mt-2">Voici un aperçu de votre activité.</p>
         </div>
             ...
             {weather && (
                 <div className="hidden md:flex items-center gap-3 bg-white px-4 py-2 rounded-2xl border border-slate-100 shadow-sm">
                     ...
                     <div>
                         ...
                         <div className="text-[10px] font-bold text-slate-400 uppercase">Météo</div>
                     </div>
                 </div>
             )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
           ...
           <div className="space-y-4">
              {posts.length === 0 ? (
                  <div className="text-center py-10 text-slate-400"><p className="font-bold mb-1">C'est calme par ici...</p><p className="text-xs italic">Créez votre premier post pour le voir apparaître.</p></div>
              ) : (
                  posts.slice(0, 3).map(post => (
                      <div key={post.id} className="flex items-start gap-4 p-4 rounded-2xl hover:bg-slate-50 transition border border-transparent hover:border-slate-100 group cursor-pointer">
                          ...
                          <div className="flex-1">
                              ...
                              <div className="flex gap-2 mt-2">{post.networks?.map(n => (<span key={n} className="text-[10px] font-bold uppercase bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-md border border-indigo-100">{n}</span>))}</div>
                          </div>
                          <div className="hidden group-hover:flex items-center justify-center w-8 h-8 rounded-full bg-white border border-slate-200 text-slate-400"><ArrowRight size={16}/></div>
                      </div>
                  ))
              )}
           </div>
        ...
        <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col items-center text-center justify-center relative overflow-hidden">
             ...
             <p className="text-xs text-slate-500 mb-6 px-4 leading-relaxed">{socialConnected ? "Vos comptes sont liés. La publication automatique est prête." : "Connectez Instagram ou Facebook pour activer le planning."}</p>
             {socialConnected ? (
                 <div className="flex items-center gap-2 text-emerald-700 font-bold bg-emerald-50 px-4 py-2 rounded-xl border border-emerald-100"><CheckCircle size={16}/> Tout est actif</div>
             ) : (
                 <button onClick={() => alert("Allez dans 'Mon Établissement' pour ajouter vos liens !")} className="w-full py-3 rounded-xl border-2 border-slate-100 font-bold text-slate-600 hover:border-indigo-600 hover:text-indigo-600 hover:bg-indigo-50 transition flex items-center justify-center gap-2 text-sm"><AlertCircle size={16}/> Configurer</button>
             )}
        </div>
      </div>
    </div>
