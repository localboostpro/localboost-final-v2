...
export default function App() {
  ...
  if (loading) return <div className="h-screen flex items-center justify-center bg-slate-50 text-slate-400 font-medium">Chargement...</div>;
  if (!session) return <AuthForm />;

  return (
    <div className="flex h-screen bg-[#F8FAFC] overflow-hidden">

      {/* SIDEBAR : Version corrigée avec séparation mobile/desktop */}
      <Sidebar
        profile={profile}
        isAdmin={isAdmin}
      ...
      <main className="flex-1 overflow-y-auto px-4 md:px-8 pt-20 md:pt-8 pb-10">
        <Routes>
          ...
          <Route path="/marketing" element={<Marketing posts={posts} profile={profile} onUpsert={upsertPostInState} onDelete={deletePostInState} />} />
          <Route path="/marketing/:id" element={<Marketing posts={posts} profile={profile} onUpsert={upsertPostInState} onDelete={deletePostInState} />} />
