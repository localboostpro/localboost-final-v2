...
// import canvasConfetti from "canvas-confetti"; // <--- J'AI COMMENTÉ CETTE LIGNE, C'EST ELLE LE COUPABLE
import {
  Wand2, Upload, Instagram, Facebook, Linkedin, 
  Save, Sparkles, Smartphone, Send, CheckCircle2
} from "lucide-react";
...
export default function Marketing({ posts, currentPost, setCurrentPost, profile, onUpdate }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.error(e);
      alert("Erreur IA (Vérifiez la clé API)");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (!error && onUpdate) {
      ...
      alert("✅ Post enregistré avec succès !"); // Remplacement des confettis
    }
  };
